...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        if (!empty($items)) {
            $stmt = mysqli_prepare($conn, "INSERT INTO camp_boq_items (boq_id, spare_name, quantity,unit_price,category) VALUES (?, ?, ?, ?, ?)");
            foreach ($items as $item) {
                ...
                mysqli_stmt_bind_param($stmt, "isiss", $boq_id, $spare_name, $quantity,$unit_price,$category);
                if (!mysqli_stmt_execute($stmt)) {
